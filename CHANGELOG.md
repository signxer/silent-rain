...
## 2.3.7
- feat(exam): 交卷前加入模拟作答延时，按「题量 × 每题随机耗时」等待后提交，避免 AI 答题瞬间交卷显得异常
- feat(exam): 考试设置新增「交卷延时」区间（秒/题），默认 10~20 秒，可填 0 关闭；单题上限 600 秒防止误填
- fix(exam): 延时期间分片等待并响应「停止学习」，停止时跳过剩余延时按原流程交卷
